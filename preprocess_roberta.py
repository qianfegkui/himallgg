...
def split():
    ...
    [speakers, labels, roberta1, sentences, train_ids, test_ids, valid_ids]\
    = pickle.load(open('data/iemocap/iemocap_features_roberta_onlylasthidden.pkl','rb'))
    ...
    for vid in tqdm(train_ids, desc="train"):
        ...
        train.append(himallgg.Sample(vid, video_speakers[vid], video_labels[vid],
                                 roberta1[vid], video_audio[vid], video_visual[vid],
                                 video_sentence[vid]))
        # print(train)
    for vid in tqdm(valid_ids, desc="dev"):
        dev.append(himallgg.Sample(vid, video_speakers[vid], video_labels[vid],
                               roberta1[vid], video_audio[vid], video_visual[vid],
                               video_sentence[vid]))
    for vid in tqdm(test_ids, desc="test"):
        # print(video_text[vid].__len__())
        # print(video_text[vid][1].shape)
        # exit()
        test.append(himallgg.Sample(vid, video_speakers[vid], video_labels[vid],
                                roberta1[vid], video_audio[vid], video_visual[vid],
                                video_sentence[vid]))
    log.info("train vids:")
    log.info(sorted(train_ids))
    log.info("dev vids:")
    log.info(sorted(valid_ids))
    log.info("test vids:")
